...
int main() {
   ...
   tuple<int , double, int> p = make_tuple(6,7.9,10); 
   cout<<get<0>(p)<<' '<<get<1>(p)<<" "<<get<2>(p)<<endl;

   tuple<pair<int, int>,double,tuple<int, char>> s = make_tuple(make_pair(5,9),5.96,make_tuple(5,'s'));
   cout<< get<0>(s).ff<<" "<<get<0>(s).ss<<" "<<get<1>(s)<<" "<<get<0>(get<2>(s))<<" "<<get<1>(get<2>(s)); 
