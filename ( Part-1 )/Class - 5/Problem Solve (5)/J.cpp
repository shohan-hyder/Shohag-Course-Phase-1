...
int main() {
   ...
   loopi(i,0,n) {
        ...
        mp[a[i]]--;
        cnt += n - i - 1 - mp[a[i]]; // -1 because index start from 0
        //map store every how many time every number apperas and also  (mp[a[i]])--  as 1 of that finish the count so need to delete
